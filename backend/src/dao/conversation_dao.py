...
class Conversation():
    def __init__(self, conversation_id, conversation_type, topic_id, doc_id, question, anwser, next_keywords, tags, last_modified_ts) -> None:
...
class ConversationDao():
    ...
    def insert_conversation(self, conversation_type, topic_id, doc_id, question, anwser, next_keywords, tags):
        doc_id = doc_id or "NULL"
        question = ("\"%s\"" % question) if question else "NULL"
        anwser = ("\"%s\"" % anwser) if anwser else "NULL"
        next_keywords = ",".join(next_keywords or [])
        tag_ids = ",".join([str(_) for _ in tag_dao.name_to_ids(tags or [])])

        vals = tuple([conversation_type, str(topic_id), str(doc_id), question, anwser, next_keywords, tag_ids])
